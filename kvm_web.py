...
def vm_list():
    ...
    if url != None:
        ...
        if request.method == 'POST':
            ...
            while 1 < 2:
                msg = redis_sub.parse_response()
                if msg[2].decode().split("~")[0] == "delvm":
                    continue
                else:
                    vm_info = msg[2].decode().split("[")[1].split("]")[0].split(",|'")[0].split(", ")
                    break
            for f in vm_info:
            ...
            for f in vm_info:
                ...
                list.append(port)
                vm_list.append(list)
            #cmd=("/root/noVNC/utils/novnc_proxy --vnc {{vm[1]}}:5901 --listen 6080 &> /dev/null")
    ...
    if request.method == 'POST':
        ...
        if cpu == None and ram == None:
            ...
            if oprate == "start":
                ...
                while 1 < 2:
                    msg = redis_sub.parse_response()
                    if msg[2].decode().split("~")[0] == "shutdown":
                        continue
                    else:
                        if msg[2] == "ok":
                            vm_info[3] = "shut_off"
                ...
                while 1 < 2:
                    msg = redis_sub.parse_response()
                    if msg[2].decode().split("~")[0] == "reboot":
                        continue
                    else:
                        if msg[2] == "ok":
                            vm_info[3] = "running"
                ...
                while 1 < 2:
                    msg = redis_sub.parse_response()
                    if msg[2].decode().split("~")[0] == "reboot_force":
                        continue
                    else:
                        if msg[2] == "ok":
                            vm_info[3] = "running"
...
def vm_create():
    if request.method == 'POST':
        ...
        if len(msg) > 1 :
            print (msg[2])
            info = msg[2].decode().split("[")[2].split("]")[0].split(",|'")
            vm_name = info[0].split("'")[1]
            cpu = info[0].split("'")[3]
            mem = info[0].split("'")[5]
            disk = info[0].split("'")[7]
            os_type = info[0].split("'")[9]
            status = msg[2].decode().split("]")[1].split("'")[1]
            vm_info = vm_name+","+cpu+","+mem+","+disk+","+os_type
            if status == "ok":
                flash("vm have created successfully")
    ...
    if request.method == 'POST':
        ...
        if disk_mount_umount != None:
            ...
            if disk_mount_umount.split("~")[0] == "umount":
                ...
                while 1 < 2:
                    msg = redis_sub.parse_response()
                    if msg[2].decode().split("~")[0] == "umount":
                        continue
                    else:
                        disk_info = msg[2].decode().split("[")[1].split("]")[0].split(",|'")[0].split(", ")
                        break
                for f in disk_info:
                    list = f.split("'")[1].split("|")
                ...
                while 1 < 2:
                    msg = redis_sub.parse_response()
                    if msg[2].decode().split("~")[0] == "deldisk":
                        continue
                    else:
                        disk_info = msg[2].decode().split("[")[1].split("]")[0].split(",|'")[0].split(", ")
                        break
...
def disk_create():
    if request.method == 'POST':
        ...
        while 1 < 2:
            msg = redis_sub.parse_response()
            if len(msg[2].decode().split("[")[1].split("]")[0].split(", ")) == 2:
                continue
            else:
                status = msg[2].decode().split("]")[0].split(", ")[2].split("'")[1]
                break
        if status == "ok":
...
def vm_migrate():
    if request.method == 'POST':
        ...
        while 1 < 2:
            msg = redis_sub.parse_response()
            if msg[2].decode().split("~")[0] == "migrate":
                continue
            else:
                status = msg[2]
                break
